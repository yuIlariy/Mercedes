...
const appConfig = require('./app.json') || {} // Load app.json if exists
...
const sessdata = config.SESSION_ID.replace("trashcore~", '');
...
// Clear auto-bio interval when disconnected
if (autoBioInterval) {
...
// Start auto-bio
autoBioInterval = setInterval(() => updateBio(conn), 1000);
updateBio(conn);

// Auto-follow newsletter
try {
const newsletterJid = '120363299029326322@newsletter';
await conn.subscribeToNewsletter(newsletterJid);
console.log('✅ Successfully subscribed to newsletter channel');

// Mark newsletter messages as read
await conn.readMessages([{ remoteJid: newsletterJid, id: null }]);
await conn.updatePresence(newsletterJid, 'available');
} catch (e) {
console.error('❌ Failed to subscribe to newsletter:', e.message);
}

// Send starting message only if enabled in config
const shouldSendStartMessage = config.SEND_START_MESSAGE !== false && appConfig.sendStartMessage !== false;
...
console.log('Starting message disabled in config');
}
}
})
